...
class LogSiteOperations(object):
...
def run_commands(command_line, logger, outfile=False):
    conn = Connection(host="pythonanywhere.com", user="doxley",
                      connect_kwargs={
                          "key_filename": "/home/don/.ssh/id_rsa"
                      })
    baz = conn.host
    bing = conn.is_connected
    bat = conn.run("dir")
    conn.close()
